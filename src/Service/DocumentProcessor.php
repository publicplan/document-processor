...
class DocumentProcessor
{
    ...
    private function handleListElement(
        ...
        ?\Publicplan\DocumentProcessor\Model\ListConfig $openListConfig,
        string &$accumulatedText
    ): array {
        $listConverter = $this->converterRegistry->findConverter($element);

        if (!$listConverter instanceof \Publicplan\DocumentProcessor\Service\Converter\ListElementConverter) {
        ...
        $listConfig = $listConverter->createListConfig($element, 0.0); // Liste selbst hat keinen bottom spacing
        $html = '';
